# Hello_Agent_Security

记录我学习 Agent 安全的过程。

## 学习什么是 Agent

Agent 可以理解为能在特定环境中完成任务的 LLM 系统，即在目标驱动下，进行计划与执行，通过工
具调用与外部世界交互，并维护状态/记忆（短期上下文、长期存储等）来迭代完成任务。
### Google Agent Study（PDF）

[谷歌 5 天人工智能代理强化课程 | Kaggle --- 5-Day AI Agents Intensive Course with Google | Kaggle](https://www.kaggle.com/learn-guide/5-day-agents)


[Introduction to Agents](agent_study/Introduction%20to%20Agents.pdf)
[Agent Tools & Interoperability with Model Context Protocol (MCP)](agent_study/Agent%20Tools%20%26%20Interoperability%20with%20Model%20Context%20Protocol%20%28MCP%29.pdf)
[Context Engineering: Sessions & Memory](agent_study/Context%20Engineering_%20Sessions%20%26%20Memory.pdf)
[Agent Quality](agent_study/Agent%20Quality.pdf)
[[Prototype to Production.pdf]]



